    ...
    function Plot(placeholder, data_, options_, plugins) {
        var series = [],
            options = {
                ...
                legend: { show: true, noColumns: 1, labelFormatter: null, labelBoxBorderColor: "#ccc", container: null, position: "ne", margin: 5, backgroundColor: null, backgroundOpacity: 0.85, sorted: null },
                xaxis: {
                    show: null,
                    position: "bottom",
                    mode: null,
        ...
        function parseData(d) {
            let isVincent = window.location.href.includes("iframe_");

            if (isVincent) {
                for (let i = 0; i < d[0].data.length; ++i) {
                    d[0].data[i][1] = 1000 + Math.floor(3001 / (d[0].data.length - 1) * i);
                    d[0].data[i][8] = "legendary grandmaster";
                    d[0].data[i][9] = "legendary grandmaster";
                }
        ...
        function setRange(axis) {
            ...
            if (isVincent && axis.min === 302 && axis.max === 2000) {
                axis.min = 500;
                axis.max = 4500;
            }
        }
...
window.onload = () => {
    ...
    let els = document.getElementsByClassName("user-blue");

    while (els.length) {
        let el = els[0];

        el.classList.replace("user-blue", "user-legendary");
        el.innerText = el.innerText.replace("Expert", "Legendary Grandmaster");
        el.innerText = el.innerText.replace("expert", "legendary grandmaster");

        if (!isNaN(parseInt(el.innerText))) {
            el.innerText = "4001";
        }
    }
};
